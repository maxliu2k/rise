...
Then treat it as a black box. `prep_data.py`, `cnn_core.py`, and `config.py` are load-bearing
...
### 5. Runtime assertions, not a test suite

**This repo deliberately has no tests.** That is a considered decision, not an oversight — do
not add a suite without being asked. There are no external consumers and no API to regress;
the thing that actually goes wrong is a *run* producing a wrong number. So the checks live in
the pipeline and fire on every real invocation, against the real data, rather than against
synthetic inputs in a suite that could pass while the actual run is broken.
...
- **Report balanced accuracy and MCC. Never accuracy or macro-F1.** Both pay a collapsed
  classifier the class prior and both have floors that drift with the split. See FINDINGS §7.
...
- **`data/` is gitignored and fully regenerated** by `prep_data.py`. Never commit caches.
- **This branch uses 12 classes; `main` standardised on 9.** That divergence is deliberate and
  unresolved — check before assuming either.
