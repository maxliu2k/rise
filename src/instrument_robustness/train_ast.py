...
from instrument_robustness.config import DATA_ROOT, INSTRUMENT_FAMILY, PIPE
...
def train(
    *,
    ...
    manifest_path = PIPE / "windows.csv"
    labels = resolve_ast_labels(manifest_path, label_names)
...
def parse_args():
    ...
    parser.add_argument(
        "--labels",
        help="Optional comma-separated label order; defaults to validated windows.csv labels",
    )
    parser.add_argument(
        "--no-class-weights",
        action="store_true",
