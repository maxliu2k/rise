...
>
> This document was measured on the `cnn-ensemble` branch, whose preprocessing is NOT the
> preprocessing on `main`. It is preserved because the *methodology and the negative results*
> remain valid and were expensive to obtain. The absolute numbers are superseded.
>
> Two differences make the figures non-comparable with anything in `artifacts/` on `main`:
>
> 1. **`power_to_db(ref=np.max)`** — every spectrogram was normalised to its own peak. On `main`
>    the log-mel uses `ref=1.0, top_db=None`. The old form clamped ~17% of a clean image and 0%
>    of a noisy one at every SNR, so the clean-to-noisy step carried a discontinuity that was not
>    noise. Section 5's sweep inherits that confound.
> 2. **Different windowing and STFT edge handling.** `main` crops to one window per source and
>    pads with `reflect`; this branch did neither.
>
> Section 1 also still reports the CNN at **0.9600 ± 0.0138 over 3 seeds**. The 5-seed value on
> this branch is **0.9578 ± 0.0137**, and the value on `main`'s pipeline is **0.9523 ± 0.0082**.
> Section 5's noise table was measured at 3 seeds and uses `clean = 0.9600` as its reference, so
> the document cannot be made self-consistent by editing the headline — it needs the sweep re-run.
>
> What survives unchanged: the pitch-group leak measurement, the tiling probes, the metric
> argument in section 7, and the finding that ensembling buys less than the seed spread. Those are
> comparisons within this branch, so the confounds above cancel.
